<!DOCTYPE html>
    <html lang=pt-br>
        <head> 
            <meta charset="uft-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title> DB Arquitetura </title>
            <link rel="stylesheet" href="../css/falha_cadastro.css">
        </head>
        <body>       
        <header>
            <a class="logo" id="inicio"> DB Arquitetura </a>
        </header>
        <div id="envio">
        <h1> FALHA EM PROCESSAR O PEDIDO! </h1> <br>
        <h5> Você deverá tentar solicitar o orçamento novamente <br> no link abaixo! </h5>
        <a href="novo_orcamento.php" id="form"> Fazer novo orçamento </a>
        </div>
        <!-- rodapé -->
        <footer id="info">
            <ul class="contact-list">
                <li class="list-item"><span class="contact-text">Porto Alegre, RS</span></li>
                <li class="list-item"><span class="contact-text">(51) 90000-0000</span></li>
                <li class="list-item"><span class="contact-text"><a href="DbArquitetura@gmail.com" title="Contato">DbArquitetura@gmail.com</a></span></li>
              </ul>
        </footer>